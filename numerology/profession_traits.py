...
# Two-part meanings: short label + detailed explanation
...
STAR_MEANINGS_DETAIL = {
    "5star": "Exceptional combination that strongly supports recognition, authority, wealth, stability and long-term success.",
    "4.5star": "Very strong combination that supports consistent progress, influence, growth and above-average results in career and life.",
    "4star": "Strong combination that is favourable for career, leadership, money and stable long-term growth with disciplined effort.",
    "3.5star": "Very good combination that provides support for opportunities, visibility and steady growth when actions are focused.",
    "3star": "Good combination that can deliver moderate, steady progress with consistent effort and practical planning.",
    "2.5star": "Average combination that requires conscious effort, discipline and patience to convert opportunities into stable success.",
    "2star": "Below-average combination where progress is slow and requires extra hard work, planning and proper guidance to stabilise.",
    "1.5star": "Struggle-oriented combination with frequent obstacles and delays, demanding patience, resilience and corrective measures.",
    "1star": "Difficult combination with high chances of setbacks, stress and instability unless remedies and discipline are applied.",
}


def _normalize_star_key(stars: str) -> str | None:
    """Normalize raw star patterns ('***', '**1/2', etc.) into keys like '3star', '3.5star'."""
    if not stars:
        return None
    ...
    if "?" in s or "(" in s:
        # special category, not a normal rating
        return None

    return None


def star_meaning_parts(stars: str) -> tuple[str, str]:
    """
    Return (short_label, detailed_text) for a given 'stars' code.

    - For standard ratings (like '3star', '3.5star', '****', '**1/2'):
      returns a two-part meaning.
    - For '?' / complex anti-combos: returns a special label + detail.
    - For unknown codes: returns ('', '').
    """
    if not stars:
        return "", ""
    ...
    # special anti / finance-issue patterns
    if "?" in s or "(" in s:
        return (
            "Challenging or unstable combination",
            "May indicate financial stress, uncertainty or unstable patterns unless handled carefully.",
        )

    key = _normalize_star_key(s)
    if not key:
        return "", ""

    short = STAR_MEANINGS_SHORT.get(key, "")
    detail = STAR_MEANINGS_DETAIL.get(key, "")

    # fallback if key was normalized but missing
    if not short and not detail:
        return (
            "Average or mixed combination",
            "Results depend strongly on personal effort, timing and support from other numbers.",
        )
...
def star_meaning(stars: str) -> str:
    """
    Backwards-compatible helper that still returns a single string,
    by joining the short + detailed parts.

    Existing code can keep using star_meaning(...),
    while the UI can use star_meaning_parts(...) for two-line display.
    """
    short, detail = star_meaning_parts(stars)
    if short and detail:
        return f"{short}; {detail}"
    return short or detail
...
def _dedupe_professions_in_pairs():
    """
    Remove duplicate professions inside each PAIRS[(mulank, bhagyank)] list.
    Preserves original labels as provided by the user, only removing exact 
    case-insensitive duplicates to maintain the rich variety of career paths.
    """
