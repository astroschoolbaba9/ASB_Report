...
def ai_summary(
    ...
    name: str | None = Query(None, description="Optional user name for personalization")
):
    ...
    """
    try:
        result = generate_interpretation(dob, name=name)
...
def relationship_ai_summary(
    ...
    name: str | None = Query(None, description="Optional name of the first person")
):
    ensure_allowed("relationship")
    try:
        # One-paragraph AI narrative (relationship tone)
        result = generate_relationship_interpretation(left, right, name=name)
    except Exception as e:
...
def ai_yearly_prediction(
    ...
    name: str | None = Query(None, description="Optional user name")
):
    ...
    """
    try:
        result = generate_yearly_interpretation(dob, year, name=name)
    except Exception as e:
...
def ai_monthly_prediction(
    ...
    result = generate_monthly_interpretation(dob, year, month, name=name)
    try:
        payload = result.model_dump()
        interpretation_text = payload.get("interpretation", "")
    except Exception:
...
def ai_daily_interpretation(
    ...
    try:
        result = generate_daily_interpretation(dob, day=day, name=name)  # ⬅ pass through
    except Exception as e:
...
def ai_health_summary(
    ...
    """
    try:
        result = generate_health_interpretation(dob, gender=gender, name=name)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"AI generation failed: {e}")
...
def ai_health_daily(
    ...
    """
    try:
        result = generate_health_daily_interpretation(dob, day=day, gender=gender, name=name)
    except Exception as e:
...
def ai_health_monthly(
    ...
    """
    try:
        result = generate_health_monthly_interpretation(dob, year, gender=gender, name=name)
    except Exception as e:
...
def ai_health_yearly(
    ...
    """
    try:
        result = generate_health_yearly_interpretation(dob, year, gender=gender, name=name)
    except Exception as e:
...
def ai_profession_summary(
    ...
    try:
        result = generate_profession_interpretation(dob, name=name)
    except Exception as e:
...
def swot_analysis(
    ...
    try:
        ...
        raw = generate_interpretation(dob=dob, name=name)
