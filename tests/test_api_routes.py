...
def test_numerology_basic_json():
    c = TestClient(app)
    r = c.get("/api/numerology/mystical-triangle.json", params={"dob": "29-10-2001"})
    assert r.status_code == 200
    js = r.json()
...
def test_triptych_png_ok():
    c = TestClient(app)
    r = c.get(
        "/api/numerology/mystical-triangle-triptych.png",
...
def test_ai_summary_mock_provider(monkeypatch):
    ...
    c = TestClient(app)
    r = c.get("/api/ai/summary", params={"dob": "29-10-2001"})
    assert r.status_code == 200
    js = r.json()
...
def test_ai_report_pdf_bytes(monkeypatch):
    ...
    monkeypatch.setattr(ai_settings, "llm_provider", "mock", raising=False)
    c = TestClient(app)
    r = c.get("/api/ai/report.pdf", params={"dob": "29-10-2001"})
    assert r.status_code == 200
